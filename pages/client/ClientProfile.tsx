...
import { User, Package, Wallet, Shield, CheckCircle, AlertTriangle, CreditCard, Camera, MapPin, ArrowLeft, Tractor, Plus, Trash2, LogOut, Star, Phone, Mail, Navigation, History, Archive, Heart, Search, Upload, FileText, X, Info, ThumbsUp, Users, Copy } from 'lucide-react';
...
export const ClientProfile: React.FC = () => {
  const { user, orders, payForOrder, confirmReceipt, reportProblem, clients, producers, updateClientProfile, upgradeClientToProducer, logout, submitReview, revealContactInfo, offers, toggleFavorite, cancelOrder, getWallet, reviews, getAverageRating } = useStore();
  ...
  const confirmPayment = () => { if (!paymentOrderId) return; const result = payForOrder(paymentOrderId); if (!result.success && result.error === 'INSUFFICIENT_FUNDS') { alert(t('order.insufficient')); } setShowPaymentRecap(false); setPaymentOrderId(null); };
  ...
  const getProducerContact = (producerId: string) => { const p = producers.find(prod => prod.id === producerId); return p ? { phone: p.phone, email: p.email } : { phone: 'N/A', email: 'N/A' }; };
  ...
  const copyReferralLink = () => { if (!currentClient?.referralCode) return; const link = `${window.location.origin}/#/register?ref=${currentClient.referralCode}`; navigator.clipboard.writeText(link); alert("Referral link copied!"); };
